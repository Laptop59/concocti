...
public class ConcoctiMachineSettingsBuildPreviewChangeC2SHandler {
    public static void handleData(final ConcoctiMachineSettingsBuildPreviewChangeC2S data, final IPayloadContext context) {
        ...
        context.enqueueWork(() -> {
                    ...
                    if (player instanceof ServerPlayer target) {
                        ...
                        if (menu instanceof ConcoctiMultiblockMenu machineMenu) {
                            if (machineMenu.containerId != data.containerId()) return; // just in case
                            machineMenu.changeBuildPreview();
                        }
                    }
                })
                .exceptionally(e -> {
