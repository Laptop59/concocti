...
public class ConcoctiEnergyGeneratorRecipe implements ProcessingRecipe<ConcoctiEnergyGeneratorRecipe, SingleRecipeInput> {
    ...
    public ConcoctiEnergyGeneratorRecipe(ResourceLocation id, Ingredient inputItem, int ticks) {
        this.inputItem = inputItem;
        this.ticks = ticks;
        idMap.put(inputItem, id);
    }

    public ConcoctiEnergyGeneratorRecipe(Ingredient inputItem, int ticks) {
        this.inputItem = inputItem;
        this.ticks = ticks;
    ...
    public @NotNull RecipeSerializer<?> getSerializer() {
        return ConcoctiRecipes.CONCOCTI_ENERGY_GENERATOR_RECIPE_SERIALIZER.get();
    }

    @Override
    public @NotNull RecipeType<?> getType() {
        return ConcoctiRecipes.CONCOCTI_ENERGY_GENERATOR_RECIPE_TYPE.get();
    }
    ...
    public static class Builder implements RecipeBuilder {
        ...
        public void save(RecipeOutput recipeOutput, @NotNull ResourceLocation id) {
            ConcoctiEnergyGeneratorRecipe recipe = new ConcoctiEnergyGeneratorRecipe(id, this.inputItem, this.ticks);
            recipeOutput.accept(id, recipe, null);
        }
    }


    public static class Serializer implements RecipeSerializer<ConcoctiEnergyGeneratorRecipe> {
        public static final MapCodec<ConcoctiEnergyGeneratorRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
                Ingredient.CODEC.fieldOf("ingredient").forGetter(ConcoctiEnergyGeneratorRecipe::getInputItem),
                Codec.INT.fieldOf("ticks").forGetter(ConcoctiEnergyGeneratorRecipe::getTicks)
        ).apply(inst, ConcoctiEnergyGeneratorRecipe::new));

        public static final StreamCodec<RegistryFriendlyByteBuf, ConcoctiEnergyGeneratorRecipe> STREAM_CODEC =
                StreamCodec.composite(
                        Ingredient.CONTENTS_STREAM_CODEC, ConcoctiEnergyGeneratorRecipe::getInputItem,
                        ByteBufCodecs.INT, ConcoctiEnergyGeneratorRecipe::getTicks,
                        ConcoctiEnergyGeneratorRecipe::new
                );

        // Return our map codec.
        @Override
        public @NotNull MapCodec<ConcoctiEnergyGeneratorRecipe> codec() {
            return CODEC;
        }

        // Return our stream codec.
        @Override
        public @NotNull StreamCodec<RegistryFriendlyByteBuf, ConcoctiEnergyGeneratorRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}